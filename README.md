...
| Value of $\lambda$ | Meaning                                                                                            |
| ------------------ | -------------------------------------------------------------------------------------------------- |
| $\lambda = 1$      | No correlation within nests → collapses to standard logit                                          |
| $\lambda \to 0$    | Perfect correlation within nests → consumers first choose a segment, then pick uniformly within it |
| $0 < \lambda < 1$  | Products in the same nest are closer substitutes than products in different nests                  |
...
1. **Price**: correlated with $\xi_j$ because firms set higher prices for higher unobserved quality
2. **$\ln s_{j|g}$ (within-nest share)**: correlated with $\xi_j$ because a product with higher unobserved quality captures a larger share within its nest
...
1. Are correlated with price and within-nest share (relevance)
2. Are uncorrelated with unobserved product quality $\xi_j$ (exogeneity)
...
1. **Estimate demand** (get $\alpha$, $\beta$, $\lambda$)
2. **Recover marginal costs** from pre-merger prices using the pre-merger ownership $\Omega^{pre}$
3. **Construct post-merger ownership** $\Omega^{post}$ (merging firms' products now internalized)
4. **Solve for new equilibrium prices** under $\Omega^{post}$ with same marginal costs
5. **Compare** pre- and post-merger prices, shares, and welfare
...
1. Start with pre-merger prices: $\mathbf{p}^{(0)} = \mathbf{p}^{pre}$
2. Compute shares and derivatives at $\mathbf{p}^{(n)}$
3. Update: $\mathbf{p}^{(n+1)} = \mathbf{c} - \Omega(\mathbf{p}^{(n)})^{-1} \mathbf{s}(\mathbf{p}^{(n)})$
4. Repeat until $\mathbf{p}^{(n+1)} - \mathbf{p}^{(n)} < \varepsilon$
...
1. **Define the new product**: assign it average characteristics of the target segment, set $\xi = 0$ (no unobserved quality advantage), and assign marginal cost equal to the segment average
2. **Expand the market**: add the new product to the choice set
3. **Solve for the new equilibrium**: all firms (including the entrant) re-optimize prices
4. **Compare profits**: is the firm better off with the new product?
...
| Concept                  | Formula                                                                         |
| ------------------------ | ------------------------------------------------------------------------------- |
| Mean utility             | $\delta_j = X_j\beta + \alpha p_j + \xi_j$                                      |
| Within-nest share        | $s_{j|g} = \exp(\delta_j/\lambda) / D_g$                                        |
| Inclusive value          | $D_g = \sum_{k \in g} \exp(\delta_k/\lambda)$                                   |
| Nest share               | $s_g = D_g^\lambda / (1 + \sum_h D_h^\lambda)$                                  |
| Overall share            | $s_j = s_{j|g} \cdot s_g$                                                       |
| Berry inversion          | $\ln s_j - \ln s_0 = X_j\beta + \alpha p_j + (1-\lambda)\ln s_{j|g} + \xi_j$    |
| BLP instrument           | $Z_{jt} = \sum_{k: f(k)\neq f(j)} X_{kt}$                                       |
| Own-price deriv.         | $\partial s_j/\partial p_j = \alpha s_j[1/\lambda(1-s_{j|g}) + s_{j|g}(1-s_g)]$ |
| Cross-price (same nest)  | $\partial s_k/\partial p_j = -\alpha s_k s_{j|g}(1/\lambda - 1 + s_g)$          |
| Cross-price (diff. nest) | $\partial s_k/\partial p_j = -\alpha s_k s_j$                                   |
| Marginal cost recovery   | $\mathbf{c} = \mathbf{p} + \Omega^{-1}\mathbf{s}$                               |
| Equilibrium fixed point  | $\mathbf{p} = \mathbf{c} - \Omega(\mathbf{p})^{-1}\mathbf{s}(\mathbf{p})$       |
